...
with open('files/students_tardiness.csv', 'r') as csvfile:
    ...
    for row in csvreader:
        ...
        data_student_tardiness = (student_id, datetime.datetime.strptime(row["tardiness_date"],'%m/%d/%y').strftime('%Y-%m-%d'), row["tardiness_code"], row["remarks"],now.strftime("%Y-%m-%d"),now.strftime("%Y-%m-%d"))
        print("Adding record")
